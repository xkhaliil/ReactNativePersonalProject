...
describe("Card – smoke", () => {
  ...
  it("renders surfaceAlt variant without crashing", () => {
    const { toJSON } = render(
      <Card variant="surfaceAlt"><Text>Alt</Text></Card>,
    )
    expect(toJSON()).not.toBeNull()
  })
  ...
  it("renders bordered variant without crashing", () => {
    const { toJSON } = render(
      <Card variant="bordered" accentColor="#1DB954"><Text>Bordered</Text></Card>,
    )
    expect(toJSON()).not.toBeNull()
  })
  ...
  it("renders leftAccent variant without crashing", () => {
    const { toJSON } = render(
      <Card variant="leftAccent" accentColor="#1DB954"><Text>Accented</Text></Card>,
    )
    expect(toJSON()).not.toBeNull()
  })
